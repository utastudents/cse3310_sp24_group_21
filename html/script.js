...
function buttonClick(i, j) {
    ...
    var wordLength = Math.abs(p1i - j) + Math.abs(p1j - i) + 1;
    var points = calculatePoints(wordLength);
    
    updatePoints(points);
    var updatePointsMsg = {
        "Action": "UPDATE_POINTS",
        "PlayerIdx": id, // Assuming id represents the player index
        "Points": points
    };
    connection.send(JSON.stringify(updatePointsMsg));
}

// Function to calculate points based on word length
function calculatePoints(wordLength) {
    // Assign points based on word length, you can adjust this based on your scoring system
    var points = wordLength * 1; // For example, 1 point per letter
    return points;
}

// Function to update points for the current player in the UI
function updatePoints(points) {
    // You can update the points display here, for example:
...
function updateLeaderboard(players) {
    ...
    // Add player names to the leaderboard
    for (var i = 0; i < players.length; i++) {
...
function startGame() {
    ...
    // Update the leaderboard with the current list of players
    ...
    // Create a new element to display player points
    var playerPointsLabel = document.createElement("label");
    playerPointsLabel.id = "playerPoints";
    playerPointsLabel.textContent = "Points: 0"; // Initial points display
    document.getElementById("container").appendChild(playerPointsLabel);
}
