...
//}
  ...
  public void onMessage(WebSocket conn, String message) {
      ...
      if (message.startsWith("WORD_SELECTED_AND_HIGHLIGHTED")) {
          // handle the message indicating that a word is selected and highlighted
          Game game = conn.getAttachment();
          UserEvent userEvent = new Gson().fromJson(message, UserEvent.class);
          
          // chhheck if the word is selected and highlighted
          if (userEvent.isWordSelectedAndHighlighted()) {
              switch (userEvent.getPlayerIdx()) {
                  case PLAYERONE:
                  ...
                  case PLAYERTWO:
                      ...
                      break;
                  case PLAYERTHREE:
                      game.setPlayerThreePoints(game.getPlayerThreePoints() + 1);
                      break;
                  case PLAYERFOUR:
                      game.setPlayerFourPoints(game.getPlayerFourPoints() + 1);
                      break;
                  default:
                      break;
              }
              
              broadcast(new Gson().toJson(game));
          }
      } else {
    ...
    private void broadcastGameStateAndLeaderboard(Game game) {
    Gson gson = new Gson();
    String gameStateJson = gson.toJson(game);
    broadcast(gameStateJson); // Broadcast updated game state
   
    String leaderboardJson = gson.toJson(game.leaderboard);
    broadcast("LEADERBOARD_UPDATE:" + leaderboardJson);
}
